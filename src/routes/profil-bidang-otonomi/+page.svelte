...
$: profilePemerintahan = data.DatasProfileBidang?.documents.find(
    ...
    $: {
        ...
        console.log('DatasGambarOtonomi structure:', data.DatasGambarOtonomi);
        if (data.DatasGambarOtonomi) {
            console.log('Files array:', data.DatasGambarOtonomi.files);
            console.log('Documents array:', data.DatasGambarOtonomi.documents);
        }
    }
    */

    // Perbaiki struktur data - coba beberapa kemungkinan struktur
    $: allImages = data.DatasGambarOtonomi?.files || 
                   data.DatasGambarOtonomi?.documents || 
                   (Array.isArray(data.DatasGambarOtonomi) ? data.DatasGambarOtonomi : []);
    /** 
    $: {
    ...
    **/
    ...
    $: koorOtda = data.pegawaiByJabatan ? data.pegawaiByJabatan['Kepala Bidang Koordinator Otonomi Daerah'] : null;
    ...
    function getImageView(fileId) {
        ...
        try {
            const bucketId = '67920a70002df0712bce';
...
<svelte:head>
    <title>Profil Bidang Koordinator Otonomi Daerah</title>
    <meta name="description" content="Profil Bidang Koordinator Otonomi Daerah Biro Pemerintahan dan Otonomi Daerah Sulawesi Tenggara" />
</svelte:head>
<br/>
 <nav class="flex">
...
<div class="page-container">
    <header class="header-card">
        ...
        <div class="title-container">
            <h1>PROFIL BIDANG KOORDINATOR OTONOMI DAERAH</h1>
...
<div class="container">
    ...
    <div class="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 p-5"> 
        <div class="w-44 h-60 bg-blue-100 rounded-lg flex-shrink-0 border p-2 mb-4 md:mb-0">
           <img src={koorOtda.URL_PhotoProfile || (koorOtda.Jenis_Kelamin === 'Wanita' ? PicProfileF : PicProfileM)} 
                class="w-full h-full object-cover rounded-md" alt="Foto Kepala Biro" 
            />
        </div>
        
         <div class="flex flex-col">
            <p class="text-xl lg:text-2xl text-gray-800"> <strong class="font-bold">{koorOtda.Nama}</strong>
            </p>
            
            <p class="text-base text-gray-600 mt-1"> NIP: {koorOtda.NIP}
            </p>
            
            <div class="mt-6">
                <h2 class="text-xl lg:text-2xl font-bold text-gray-900 leading-tight">
                    KEPALA BIDANG KOORDINATOR OTONOMI DAERAH
                </h2>
            </div>
        </div>
    </div>
    ...
    <TabItem open title="Tupoksi">
      <p class="preserve-lines text-2sm text-gray-500 dark:text-gray-400">  
         { profilePemerintahan.tupoksi }
      </p>
    </TabItem>
    <TabItem title="Program Prioritas">
      <p class="preserve-lines text-2sm text-gray-500 dark:text-gray-400">
       { profilePemerintahan.programPrioritas }
      </p>
    </TabItem>
  </Tabs> 
     {:else}
    ...
    {#if totalImages === 0}
        <div class="no-images-container">
            <p class="text-gray-600 text-lg">Saat ini tidak terdapat gambar pada Galeri Foto Bidang Koordinator Otonomi Daerah.</p>
            <p class="text-sm text-gray-500 mt-2">Periksa koneksi storage atau pastikan gambar telah diupload.</p>
        </div>
    {:else}
        <p class="mb-4 text-gray-700">Menampilkan {paginatedImages.length} dari {totalImages} gambar foto kegiatan Bidang Koordinator Otonomi Daerah.</p>
