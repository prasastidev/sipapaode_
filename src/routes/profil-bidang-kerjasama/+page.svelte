...
$: profilePemerintahan = data.DatasProfileBidang?.documents.find(
    (profil) => profil.namaBidang === "Bagian Kerjasama"
);
    ...
    $: {
        ...
        console.log('DatasGambarKS structure:', data.DatasGambarKS);
        if (data.DatasGambarKS) {
            console.log('Files array:', data.DatasGambarKS.files);
            console.log('Documents array:', data.DatasGambarKS.documents);
        }
    }
    **/

    // Perbaiki struktur data - coba beberapa kemungkinan struktur
    $: allImages = data.DatasGambarKS?.files || 
                   data.DatasGambarKS?.documents || 
                   (Array.isArray(data.DatasGambarKS) ? data.DatasGambarKS : []);
     /** 
    $: {
    ...
    **/
    ...
    $: kabidKS = data.pegawaiByJabatan ? data.pegawaiByJabatan['Kepala Bidang Kerjasama'] : null;
    ...
    function getImageView(fileId) {
        ...
        try {
            const bucketId = '67545c85000bd5d33ef2';
...
<svelte:head>
    <title>Profil Bidang Kerjasama</title>
    <meta name="description" content="Profil Bidang Kerjasama Biro Pemerintahan dan Otonomi Daerah Sulawesi Tenggara" />
</svelte:head>
...
<div class="page-container">
    <header class="header-card">
        ...
        <div class="title-container">
            <h1>PROFIL BIDANG BAGIAN KERJASAMA</h1>
...
<div class="container">
    ...
    {:else}
        <p class="text-center text-gray-500">
            Data profil untuk "Bagian Kerjasama" tidak ditemukan.
        </p>
    {/if}
    ...
    <div class="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 p-5"> 
        <div class="w-44 h-60 bg-blue-100 rounded-lg flex-shrink-0 border p-2 mb-4 md:mb-0">
           <img src={kabidKS.URL_PhotoProfile || (kabidKS.Jenis_Kelamin === 'Wanita' ? PicProfileF : PicProfileM)} 
                class="w-full h-full object-cover rounded-md" alt="Foto Kepala Biro" 
            />
        </div>
        
         <div class="flex flex-col">
            <p class="text-xl lg:text-2xl text-gray-800"> <strong class="font-bold">{kabidKS.Nama}</strong>
            </p>
            
            <p class="text-base text-gray-600 mt-1"> NIP: {kabidKS.NIP}
            </p>
    ...
    {#if totalImages === 0}
        <div class="no-images-container">
            <p class="text-gray-600 text-lg">Saat ini tidak terdapat gambar pada Galeri Foto Bagian Kerjasama.</p>
            <p class="text-sm text-gray-500 mt-2">Periksa koneksi storage atau pastikan gambar telah diupload.</p>
        </div>
    {:else}
        <p class="mb-4 text-gray-700">Menampilkan {paginatedImages.length} dari {totalImages} gambar foto kegiatan bidang bagian kerjasama.</p>
