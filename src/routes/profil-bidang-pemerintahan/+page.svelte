...
$: profilePemerintahan = data.DatasProfileBidang?.documents.find(
    ...
    **/
    ...
    $: allImages = data.DatasGambarPemerintahan?.files || 
                   data.DatasGambarPemerintahan?.documents || 
                   (Array.isArray(data.DatasGambarPemerintahan) ? data.DatasGambarPemerintahan : []);
    ...
    **/
    ...
    $: koorPemerintahan = data.pegawaiByJabatan ? data.pegawaiByJabatan['Kepala Bidang Koordinator Pemerintahan'] : null;
...
<div class="page-container">
    <header class="header-card">
        ...
        <div class="title-container">
            <h1>PROFIL BIDANG KOORDINATOR PEMERINTAHAN</h1>
   ...
   {#if profilePemerintahan}
    ...
    <div class="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 p-5"> 
        <div class="w-44 h-60 bg-blue-100 rounded-lg flex-shrink-0 border p-2 mb-4 md:mb-0">
           <img src={koorPemerintahan.URL_PhotoProfile || (koorPemerintahan.Jenis_Kelamin === 'Wanita' ? PicProfileF : PicProfileM)} 
                class="w-full h-full object-cover rounded-md" alt="Foto Kepala Biro" 
            />
        </div>
        
         <div class="flex flex-col">
            <p class="text-xl lg:text-2xl text-gray-800"> <strong class="font-bold">{koorPemerintahan.Nama}</strong>
            </p>
            
            <p class="text-base text-gray-600 mt-1"> NIP: {koorPemerintahan.NIP}
            </p>
            
            <div class="mt-6">
                <h2 class="text-xl lg:text-2xl font-bold text-gray-900 leading-tight">
                     KEPALA BIDANG KOORDINATOR PEMERINTAHAN
                </h2>
            </div>
        </div>
    </div>
    ...
    {#if totalImages === 0}
        <div class="no-images-container">
            <p class="text-gray-600 text-lg">Saat ini tidak terdapat gambar pada Galeri Foto Bidang Koordinator Pemerintahan.</p>
            <p class="text-sm text-gray-500 mt-2">Periksa koneksi storage atau pastikan gambar telah diupload.</p>
        </div>
    {:else}
        <p class="mb-4 text-gray-700">Menampilkan {paginatedImages.length} dari {totalImages} gambar foto kegiatan Bidang Koordinator Pemerintahan.</p>
